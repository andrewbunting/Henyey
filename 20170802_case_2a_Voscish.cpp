...
//        cout << 1/(radius_cm[k+1] - radius_cm[k]) << "\n";
        ...
        Mr[0][0][0] = (   ( 3.0 + rmid_cm_HR[1]*( lnRho_HR[1] - lnRho_HR[0] )/( radius_cm_HR[1] - radius_cm_HR[0] ) ) * 0.5*( a[0] + a[1]  )  +  rmid_cm_HR[1]*( a[1] - a[0] )/( radius_cm_HR[1] - radius_cm_HR[0] )  +  ( (1.0/chiRho_HR[1]) - (pressure_HR[1]/rho_HR[1])*(l*(l+1.0)/(rmid_cm_HR[1]*rmid_cm_HR[1]*m*m*omega*omega)) )*c[1]  -  (chiT_HR[1]/chiRho_HR[1])*d[1]   )*( (rmid_cm_HR[1]*rmid_cm_HR[1]) / (R*R) )*( (rmid_cm_HR[1]*rmid_cm_HR[1]) / (R*R) );
        Mr[0][1][0] = (   (1.0/( m*omega*rho_HR[1]*temperature_HR[1]*cp_HR[1]*rmid_cm_HR[1]*rmid_cm_HR[1] ))*( ( rmid_cm_HR[1]*(flux_HR[0] + flux_HR[1]) + rmid_cm_HR[1]*rmid_cm_HR[1]*(flux_HR[1] - flux_HR[0])/(radius_cm_HR[1] - radius_cm_HR[0]) )*0.5*(b[0] + b[1]) + rmid_cm_HR[1]*rmid_cm_HR[1]*0.5*(flux_HR[0] + flux_HR[1])*(b[1] - b[0])/(radius_cm_HR[1] - radius_cm_HR[0])  )   )*( (rmid_cm_HR[1]*rmid_cm_HR[1]) / (R*R) );
        
        Mi[0][0][0] = 0;
        Mi[0][1][0] = 0; // (   brunt_A_HR[1] * (chiRho_HR[1]/chiT_HR[1]) * 0.5 * ( a[0] + a[1] )  -  grada_HR[1]*c[1]  + d[1]   )*( (rmid_cm_HR[1]*rmid_cm_HR[1]) / (R*R) );
        
        
        Nr[0][0][0] = ( (rmid_cm_HR[1] - rmid_cm_HR[0])/(temperature_HR[1] - temperature_HR[0]) )*( flux_HR[0]/(delta[0]*K_HR[0] + delta[1]*K_HR[1]) )*b[0]   -   (1.0/(delta[0]*chiRho_HR[0] + delta[1]*chiRho_HR[1]))*( 1.0 + dkap_dlnrho_face_HR[0]/(delta[0]*opacity_HR[0] + delta[1]*opacity_HR[1]) )*( delta[0]*c[0] + delta[1]*c[1] )   +   ( 4.0 - (dkap_dlnT_face_HR[0]/(delta[0]*opacity_HR[0] + delta[1]*opacity_HR[1])) + ( delta[0]*(chiT_HR[0]/chiRho_HR[0]) + delta[1]*(chiT_HR[1]/chiRho_HR[1]) ) )*( 1.0 + dkap_dlnrho_face_HR[0]/(delta[0]*opacity_HR[0] + delta[1]*opacity_HR[1]) )*(delta[0]*d[0] + delta[1]*d[1])   +   (d[1] - d[0])/(lnT_HR[1] - lnT_HR[0]);
        Nr[0][1][0] = (m*m*omega*omega*radius_cm_HR[0]/(delta[0]*grav_HR[0] + delta[1]*grav_HR[1]))*a[0]   -   (1.0/( delta[0]*chiRho_HR[0] + delta[1]*chiRho_HR[1] ))*( delta[0]*c[0] + delta[1]*c[1] )   -   (1.0/(delta[0]*grav_HR[0]*rho_HR[0] + delta[1]*grav_HR[1]*rho_HR[1]))*( (pressure_HR[1]*c[1] - pressure_HR[0]*c[0])/(rmid_cm_HR[1] - rmid_cm_HR[0]) )   +   ( (delta[0]*chiT_HR[0]/chiRho_HR[0]) + (delta[1]*chiT_HR[1]/chiRho_HR[1]) )*(delta[0]*d[0] + delta[1]*d[1]);
        ...
        if (k == J-2) {
            ...
            etar[0][0][0] = 0.5*rmid_cm_HR[1]*dp_dr_BC/pressure_HR[1];
            etar[0][0][1] = 0;
            etar[0][1][0] = ( rmid_cm_HR[1]/2.0 )*( ( (log(flux_HR[1]) - log(flux_HR[0]))/(radius_cm_HR[1] - radius_cm_HR[0]) )  -  4.0*dlnT_dr_BC );
            etar[0][1][1] = 0.5;
            ...
            mur[0][0][0] = 0.5*rmid_cm_HR[1]*dp_dr_BC/pressure_HR[1];
            mur[0][0][1] = 0;
            mur[0][1][0] = ( rmid_cm_HR[1]/2.0 )*( ( (log(flux_HR[1]) - log(flux_HR[0]))/(radius_cm_HR[1] - radius_cm_HR[0]) )  -  4.0*dlnT_dr_BC );
            mur[0][1][1] = 0.5;
            ...
            xr[0][0][0] = 0.5*(a[0] + a[1])  +  c[1];
            xr[0][1][0] = 0.5*(a[0] + a[1])  +  0.5*(b[0] + b[1])  +  d[1];
...
//        cout << "\t Done the v_{" << k << "} and u_{" << k << "} loop \n";
    ...
    ofstream outfile;
    outfile.open("Output/20170802_2a_Voscish.dat", ios::out);
