...
int main()
{
    ...
    ifstream infile;
    infile.open("Input/profiles_conv_basic.txt");
    ...
    ifstream linefile;
    linefile.open("Input/profiles_conv_basic.txt");
    ...
    while (infile) {
            ...
            flux[z] = input; // this is now the radiative flux, as it has read flux_rad from the input file
...
*/
    ...
    for (k=0; k < J-1; k=k+1) {
        ...
        */
        ...
        test[k] =  (R - rmid_cm_HR[0]) * opacity_HR[0] * rho_HR[0];  // 1.0/(R * opacity_HR[0] * rho_HR[0]); // - pressure_HR[0] / ( ( (pressure_HR[1] - pressure_HR[0])/(rmid_cm_HR[1] - rmid_cm_HR[0]) ) * R ); //(radius_cm_HR[1] - radius_cm_HR[0])/R;
        test[k+1] = test[k];
...
//        cout << "\t Done the v_{" << k << "} and u_{" << k << "} loop \n";
    ...
    ofstream outfile;
    outfile.open("Output/Henyey_conv.dat", ios::out);
...
// This outputs the necessary data for the photometry file
    ...
    // This bit opens the file to write the data into
    ofstream photodata;
    photodata.open("Output/photo_data.dat", ios::out);
