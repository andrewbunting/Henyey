...
int main()
{
    ...
    // These variables were previously introduced just after the matrix loop, and just before the output section
    ...
    double dKE, dKE_eq, V_eq, h, KE_core_eq, KE_core, KE_env_eq, KE_env, KE_tot, KE_tot_eq, D_0, omega_0;
    
    double ds_prime_dr_eq, ds_prime_ds_0_eq;
    ...
    //
    //
    // This is where I start the loop for investigating the frequency dependence precisely
    //
    //
    //
    //
    ...
    int omega_loop_count, omega_loop_max;
    
    double omega_start, omega_finish, omega_step, omega_mult, omega_loop_max_doub;

    string omega_filename, omega_comparison_filename;
    
    
    omega_loop_max = 1;//4000;
    
    omega_loop_max_doub = (double) omega_loop_max;
    
    omega_start = 1.0; //1.00026; //0.99;
    
    omega_finish = 1.0; //1.00027; //1.01;
    
    omega_mult = omega_start;
    
    omega_step = (omega_finish - omega_start) / omega_loop_max_doub;
    
    cout << "omega_step = " << omega_step << "\n";
    
    
    
    ofstream search_file;
    
    search_file.open("Output/search_file.dat");
    
    search_file.precision(10);
    
    search_file << "omega_loop_count" << "\t\t" << "D / D_{0}" << "\t\t" << "omega / omega_{0}" << "\t\t" << "KE_tot" << "\t\t" << "KE_eq" << "\n";
    
    
    
for (omega_loop_count=0; omega_loop_count < omega_loop_max; omega_loop_count++) {
    
    // This needs to read from the start each time this loop stars
    ...
    D_0 = 0.0512; //*1.118587; // Orbital radius of planet in AU, 1.58740105197 = 4^(1/3), which acts to double the period of the orbit
    
    D_0 = 1.495978921e13 * D_0; // Converted into cm
    
    omega_0 = sqrt(G * ((Mstar + mp)/(D_0*D_0*D_0)));
    ...
    // This increments it for the next one
    omega_mult = omega_mult + omega_step;
    ...
    cout << "Step " << omega_loop_count+1 << " of " << omega_loop_max << "; omega = " << omega << "\n\n";
    
    //cout << "\n\n" << "for JP, f_{JP} = " << sqrt(4.0*omega*omega*R*R*R/(G*Mstar)) << "\n\n";
    ...
    //cout << "\nThis is alphar_{J-1}\n";
    //cout << alphar[J-1][0][0] << "\t" << alphar[J-1][0][1] << "\n";
    //cout << alphar[J-1][1][0] << "\t" << alphar[J-1][1][1] << "\n\n";
    ...
    */
    ...
    omega_filename = "Output/Henyey_BPT_freq_search_" + to_string(omega_loop_count) + ".dat";

    omega_comparison_filename = "Output/comparison_file_freq_search_" + to_string(omega_loop_count) + ".dat";
    
    
    // This bit opens the file to write the data into
    ofstream outfile;
    outfile.open(omega_filename, ios::out);
    ...
    comparison_file.open(omega_comparison_filename, ios::out);
    ...
    for (k=0; k<J; k=k+1) {
        ...
        // The strange stuff with the term with dgrr_dr is because I previously calculated the d(g/r^{2})/dr, when I now need d(r^{2}/g)/dr
        V_eq = ( G * mp / (4.0 * D * D * D * l * (l + 1.0)) )   *   (    (2.0*radius_cm_HR_output[k]*radius_cm_HR_output[k]/grav_HR_output[k])    +    (radius_cm_HR_output[k] * (-1.0 * ( (radius_cm_HR_output[k]*radius_cm_HR_output[k]*radius_cm_HR_output[k]*radius_cm_HR_output[k]) / (grav_HR_output[k]*grav_HR_output[k]) ) * dgrr_dr) )    );
        
        h = ( G * mp / (4.0 * D * D * D ) )  *  ( (radius_cm_HR_output[k] * radius_cm_HR_output[k]) / grav_HR_output[k] );
        
        
        if ( k == J-1 ) {
            
            dKE = 9.0 * 3.1415926 *  m * m * omega * omega * (16.0/5.0) * ( radius_cm_HR_output[k] - rmid_cm_HR_output[k] ) * rho_face_HR_output[k] * radius_cm_HR_output[k] * radius_cm_HR_output[k] * (   ( R * R * ( ur[k][0][0] * ur[k][0][0] + ui[k][0][0] * ui[k][0][0] ) )/3.0   +   2.0 * ( xi_h_real*xi_h_real  +  xi_h_imaginary*xi_h_imaginary )   );
            
            dKE_eq = 9.0 * 3.1415926 *  m * m * omega * omega * (16.0/5.0) * ( radius_cm_HR_output[k] - rmid_cm_HR_output[k] ) * rho_face_HR_output[k] * radius_cm_HR_output[k] * radius_cm_HR_output[k] * (   ( ( h*h ) )/3.0   +   2.0 * ( V_eq*V_eq )   );
            
        } else {
            
            
            dKE = 9.0 * 3.1415926 *  m * m * omega * omega * (16.0/5.0) * ( rmid_cm_HR_output[k+1] - rmid_cm_HR_output[k] ) * rho_face_HR_output[k] * radius_cm_HR_output[k] * radius_cm_HR_output[k] * (   ( R * R * ( ur[k][0][0] * ur[k][0][0] + ui[k][0][0] * ui[k][0][0] ) )/3.0   +   2.0 * ( xi_h_real*xi_h_real  +  xi_h_imaginary*xi_h_imaginary )   );
            
            dKE_eq = 9.0 * 3.1415926 *  m * m * omega * omega * (16.0/5.0) * ( rmid_cm_HR_output[k+1] - rmid_cm_HR_output[k] ) * rho_face_HR_output[k] * radius_cm_HR_output[k] * radius_cm_HR_output[k] * (   ( ( h*h ) )/3.0   +   2.0 * ( V_eq*V_eq )   );
        ...
        ds_prime_ds_0_eq = 0.0;
        
        if (brunt_A_HR_output[k] < 0.0) {
            
            ds_prime_dr_eq =  - ( ( ((G * mp * rmid_cm_HR_output[k+3])/(4.0*D*D*D*grav_HR_output[k+3]))  *  cp_HR_output[k+3]  *  brunt_A_HR_output[k+3]  *  (chiRho_HR_output[k+3] / chiT_HR_output[k+3]) )     -     ( ((G * mp * rmid_cm_HR_output[k-3])/(4.0*D*D*D*grav_HR_output[k-3]))  *  cp_HR_output[k-3]  *  brunt_A_HR_output[k-3]  *  (chiRho_HR_output[k-3] / chiT_HR_output[k-3]) )  )     /     (     rmid_cm_HR_output[k+3]  -  rmid_cm_HR_output[k-3]     );
            
            ds_prime_ds_0_eq = ds_prime_dr_eq  /  (   cp_HR_output[k]  *  ( brunt_A_HR_output[k] / radius_cm_HR_output[k] )  *  ( chiRho_HR_output[k] / chiT_HR_output[k] )   );
    ...
    search_file << omega_loop_count << "\t\t" << D / D_0 << "\t\t" << omega / omega_0 << "\t\t" << KE_tot << "\t\t" << KE_tot_eq << "\n";
    ...
    //
    //
    // We have just closed the big omega loop
    //
    //
    //
    //
    ...
    flux_file.open("Output/flux_file.dat");
    ...
    for (k=0; k<Jold; k++) {
        ...
        if (brunt_A[k] < 0.0) {
            ...
            ds_prime_dr_eq =  - ( ( ((G * mp * rmid_cm[k+20])/(4.0*D*D*D*grav[k+20]))  *  cp[k+20]  *  brunt_A[k+20]  *  (chiRho[k+20] / chiT[k+20]) )     -     ( ((G * mp * rmid_cm[k-19])/(4.0*D*D*D*grav[k-19]))  *  cp[k-19]  *  brunt_A[k-19]  *  (chiRho[k-19] / chiT[k-19]) )  )     /     (     rmid_cm[k+20]  -  rmid_cm[k-19]     );
            
            ds_prime_ds_0_eq = ds_prime_dr_eq  /  (   cp[k]  *  ( brunt_A[k] / radius_cm[k] )  *  ( chiRho[k] / chiT[k] )   );
            
        }
        
        
        flux_file << radius_cm[k] / R << "\t\t" << ds_prime_ds_0_eq << "\t\t" << (   cp[k]  *  ( brunt_A[k] / radius_cm[k] )  *  ( chiRho[k] / chiT[k] )   ) << "\n";
